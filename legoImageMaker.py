...
def processImage(album):
    ...
    for i in range(size[0]):
        for j in range(size[1]):
            ...
            if coloursUsed[colour][1] > 0:
                drawNumbers.text(
                    (i * factor + 50, j * factor + 50),
                    str(coloursUsed[colour][1]),
                    font=numberTileFont,
                    anchor="mm",
                    fill=textColour,
                )
    ...
    for key, values in coloursUsed.items():
        if values[1] > 0:
            ...
            draw.ellipse(
                (12, 2 + (25 * values[1]), 33, 23 + (25 * values[1])), fill=fill
            )
            name = str(key)
            name = name.replace("_", " ")
            draw.text(
                (40, 3 + (25 * values[1])),
                ": " + name + ", " + str(coloursUsed[key][0]),
                font=font,
                fill=textColour,
            )
...
def makeInstructions(name):
    ...
    for key, values in coloursUsed.items():
        if values[1] > 0:
            ...
            draw.text(
                (40, 3 + (25 * values[1])),
                str(values[1]) + ": " + name + ", " + str(coloursUsed[key][0]),
                font=font,
                fill=textColour,
            )
    ...
    drawText.text(
        xy=(425, 25), text=name, anchor="mm", font=ImageFont.truetype("GILSANUB", 25)
    )
    instructionPDF.drawInlineImage(titleImage, x=0, y=550)
    instructionPDF.drawInlineImage(
        newImage,
        (850 - 500 / height * width) / 2,
        50,
    ...
    for i in range(width):
        for j in range(height):
            ...
            (left, upper, right, lower) = (
                i * size[0] * factor / width,
                j * size[1] * factor / height,
                (i + 1) * size[0] * factor / width,
                (j + 1) * size[1] * factor / height,
            )
            ...
            instructionPDF.drawInlineImage(
                colourNumbersImage, 50, 50, width=200, height=500
            )
            instructionPDF.showPage()
            count = count + 1
